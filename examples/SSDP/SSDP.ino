#include <EtherCard.h> //                                                                                                                                                 |Mac address|
const char SSDP_RESPONSE[] PROGMEM = "HTTP/1.1 200 OK\r\nCACHE-CONTROL: max-age=1200\r\nEXT:\r\nSERVER:Arduino\r\nST: upnp:rootdevice\r\nUSN: uuid:abcdefgh-7dec-11d0-a765-4C6165747565\r\nLOCATION: http://"; // dont forget our MAC address USN: uuid:abcdefgh-7dec-11d0-a765-Mac addr 
...
//  in XML_DESCRIP // <presentationURL>/</presentationURL> // adress of the page who would open on service double click, you could use http://ip  but if you use DHCP it's better so don't waste memory
...
static byte ssdp[] = { 
  239,255,255,250 };
static byte mymac[] = { 
  0x4C, 0x61, 0x65, 0x74, 0x75, 0x65 }; // if you change it you must update SSDP_RESPONSE and XML_DESCRIP
...
void addip(int udppos) { // add current IP to the request and send it
  int adr;
  for(int i=0;i<4;i++) { // extract the current IP of the Arduino/Genuino
  ...
  udppos--;
  EtherCard::udpTransmit(udppos-UDP_DATA_P); // send all to the computer who make the request on the IP and port who made the request
}
